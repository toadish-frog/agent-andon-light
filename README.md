...
5 units fully assembled — custom PCB Rev A + 3D-printed enclosure, both back from JLC as of 2026-07-30 — individually flashed and tested. `G`/`Y`/`R`/`C`, the sectioned pixel layout, and the watchdog `StalePulse` fallback are all confirmed on the fabricated hardware; enclosure fit was correct first-try on every unit. Packaging & distribution: published on PyPI, and the Windows one-click installer is built and verified on real hardware; a tagged GitHub release is the last remaining step. See [`device/docs/Implementation-Summary.md`](device/docs/Implementation-Summary.md) for the full phase-by-phase status.
