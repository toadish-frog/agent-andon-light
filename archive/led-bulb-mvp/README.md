...
**Why it's here instead of deleted:** the firmware and docs in this folder are real, working, confirmed-on-hardware artifacts (see `docs/Implementation-Summary.md` §4.1/§5 Phases 0–4) — the actual history of how the wire protocol, watchdog, and hook mapping were designed and validated. Restructured 2026-07-14 from a top-level parallel `led-bulb/` directory (as if it were a second product variant) into this archive, once the WS2812 strip's custom PCB — the actual final deliverable — went to manufacturing. See `../../device/docs/Implementation-Summary.md` §5 "Phase 0–4 history" for the current project's framing of this phase.
