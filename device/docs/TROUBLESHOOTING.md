...
On the **fabricated PCB**, MCU and LEDs share one board — there's no wire or connector to inspect, so "signal integrity" problems are almost always a bad solder joint. On the **breadboard build**, the same symptoms are usually the jumper wiring instead. Both are covered below.

| Symptom | Likely cause | Fix |
| --- | --- | --- |
| Some pixels in the active section are garbled/dark, others correct | Data-line signal integrity — the RP2040 drives the data line at 3.3V logic; a marginal connection can lose sync partway down the chain | **Fabricated PCB:** check for a cold/bad solder joint on the MCU's pads. **Breadboard:** shorten the `S` wire, check the connector crimp, or add a 74HCT125 level shifter (see [`BOM.md`](BOM.md) item #7) |
| Dim colors, wrong hue, or the board browns-out/resets when the light comes on | Insufficient 5V supply to the LEDs | **Fabricated PCB:** check the USB-C cable is data *and* power capable, and the solder joints on the power pads. **Breadboard:** confirm `V` is wired to `5V`/`VBUS`, not `3V3` — see [`USER-GUIDE.md`](USER-GUIDE.md) Pitfalls to Avoid |
| Nothing lights at all — not even pixel 1 — but `andon-light set idle` returns exit code 0 | `kDataPin` in `andon_light_firmware.ino` doesn't match the actual data pin | **Fabricated PCB:** confirm `kDataPin` still matches the board's designed trace (`GPIO1` by default — don't edit this unless you changed the PCB design). **Breadboard:** check the pin you wired against `kDataPin`. Either way, this is the pin, not the LEDs — if pixel 1 lights but sections 2-10 stay dark regardless of command, that's Step 3, not this. |
| Lit pixels don't line up with the expected sections (e.g. `G` lights pixels 4-6 instead of 2-4) | The strip's data-in end doesn't match the firmware's assumption | **Fabricated PCB:** this would mean a fab/design defect — check against a known-good unit before assuming it's yours. **Breadboard:** check which end of the strip PCBA is silkscreened as the data input, confirm `S` is wired there |
